...
def init_plot():
    ...
    colorbar_plot = figure(
        width=60, toolbar_location=None, outline_line_width=0, name="colorbar_plot"
    )
...
def get_cds(som, meta_df):
    ...
    for j in range(n):
        for i in range(m):
            ...
            node_topics_pct[(i, j)] = counter1.most_common()[0][1] / len(
                node_topics[(i, j)]
            )
...
def make_plot_umatrix(plot, main_source, umatrix_long, m, n, p, palette="Viridis256"):
    ...
    umatrix_cmap = LinearColorMapper(
        palette=palette, low=min(umatrix_long), high=max(umatrix_long)
    )
    ...
    um_hex = plot.scatter(
        ...
        size=size + 1,
        alpha=1,
        fill_color={"field": "umatrix_long", "transform": umatrix_cmap},
        line_color={"field": "umatrix_long", "transform": umatrix_cmap},
...
def make_plot_hits(plot, main_source):
    ...
    """
    hits_switch = Switch(active=True, stylesheets=[hits_style])

    hits_switch.js_on_change(
        "active",
        CustomJS(args=dict(hits=hits_hex), code="""hits.visible = !hits.visible"""),
    )
...
def make_plot_topics(plot, main_source, json_path, m, n, p):
    ...
    bg_hex = plot.scatter(
        ...
        size=size,
        color=factor_cmap(
            "topics", palette=cc.palette["glasbey_category10"], factors=topic_list
        ),
        line_join="miter",
        alpha=0.9,
        visible=False,
    ...
    wedge_switch.js_on_change(
        "active", CustomJS(args=dict(w=bg_wedge), code="""w.visible = !w.visible""")
    )
...
def make_plot_topoverlay(plot, vec_df, m, n, topic_labels, main_source, cds_dict, cb):
    ...
    tb_df_long["pct"] = tb_df_long.apply(
        lambda row: np.max([0.1, (row["value"] / cds_dict["hits_long"][row["bmu"]])]),
        axis=1,
    )
    ...
    topic_input_label = [
        (t, t + " - " + " ".join(topic_labels[t].split("_")[1:])) for t in topic_labels
    ]
    ...
    to_dropdown.js_on_change(
        "value",
        CustomJS(
            args=dict(
                to_src=to_source, to_view=t0_view, to_index=IndexFilter(indices=[])
            ),
...
def make_recency_plot(plot, m, n, main_source, cds_dict):
    ...
    recency_cmap = LinearColorMapper(
        palette=rc_palette, low=min(cds_dict["recency"]), high=max(cds_dict["recency"])
    )

    recency_hex = plot.scatter(
        source=main_source,
        size=size,
        fill_color={"field": "recency", "transform": recency_cmap},
        line_join="miter",
        line_color="#999999",
...
def make_query_hex(plot, m, n, x2, y2):
    ...
    highlight_src = ColumnDataSource(
        dict(x=x2, y=y2, rank=highlight_rank, highlight=highlight_nodes)
    )
    highlight_view = CDSView(filter=IndexFilter(indices=[]))

    # highlight hex plot
    highlight_hex = plot.scatter(
...
def make_plots(vec_df, som, json_path, help_, cb):
    ...
    plot, um_hex, ub_cb = make_plot_umatrix(
        plot, main_source, cds_dict["umatrix_long"], m, n, p
    )

    plot, tp_hex, tp_wedge, topic_labels, wedge_switch = make_plot_topics(
        plot, main_source, json_path, m, n, p
    )

    colorbar_plot.add_layout(ub_cb, "right")

    plot, to_bg_hex, to_pct_hex, to_dropdown = make_plot_topoverlay(
        plot, vec_df, m, n, topic_labels, main_source, cds_dict, cb
    )

    plot, recency_hex, rc_colorbar = make_recency_plot(
        plot, m, n, main_source, cds_dict
    )

    colorbar_plot.add_layout(rc_colorbar, "right")

    plot, highlight_src, highlight_hex = make_query_hex(
        plot, m, n, cds_dict["x"], cds_dict["y"]
    )

    plot, hits_hex, hits_switch = make_plot_hits(plot, main_source)
    hex_toggle_switch = row(hits_switch, Div(text="Toggle Hits Hex"))
    wedge_toggle_switch = row(
        wedge_switch,
        Div(text="Toggle Wedge"),
        visible=False,
        name="wedge_toggle_switch",
    )

    plot_renderers = [um_hex, tp_hex, to_bg_hex, recency_hex]
    ...
    dropdowns_ = {"hex_dropdown": hex_dropdown, "to_dropdown": to_dropdown}

    highlights_ = {"highlight_src": highlight_src, "highlight_hex": highlight_hex}

    return plots_, main_source, codebook_long, dropdowns_, switches_, highlights_


def make_tables(vec_df, meta_cols, table_cols_attrs, cb):
    ...
    table_cols = [
        TableColumn(field=i, title=i, width=table_cols_attrs[i]["width"])
    ...
    node_table = DataTable(
        source=table_cds, columns=table_cols, view=table_view, index_position=-1
    )

    node_detail_text = "Click on  a row for details"
    node_detail = Div(text=node_detail_text, styles=placeholder_style)

    table_cds.selected.js_on_change(
        "indices",
        CustomJS(
            args=dict(table_src=table_cds, node_div=node_detail), code=cb["row_click"]
        ),
    )
...
def make_bars(vec_df, filter_col="user"):
    ...
    top10_users_bar = (
        bar_df_long.groupby(filter_col)
...
def get_query_buttons(transformer, codebook_tree, highlights_, m, n):
    def calculate_query(event):
        topnn = codebook_tree.query(transformer.encode(query_input.value), TOPNN_K)[
            1
        ].tolist()
    ...
    run_query_button = Button(
        label="Run Query", button_type="success", styles=Styles(**BUTTON_STYLES)
    )
...
def get_help(hc, ht):
    ...
    help_col = column(help_title, help_body, name="help_text")

    help_ = {"contents": hc, "body": help_body, "title": help_title}
