...
plots_, main_source, codebook_long, dropdowns_, switches_, highlights_ = make_plots(
    meta_df, som, json_path, help_, CALLBACKS
)
...
main_source.selected.js_on_change(
    "indices",
    CustomJS(
        args=dict(
...
run_query_button, clear_query_button, query_input = get_query_buttons(
    transformer, codebook_tree, highlights_, m, n
)
...
node_layout = column(
    bar_plot, node_detail, node_table, width=PLOT_WIDTH, height=PLOT_HEIGHT
)
